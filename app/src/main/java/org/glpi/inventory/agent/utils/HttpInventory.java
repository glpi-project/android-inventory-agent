...
public class HttpInventory {
    ...
    private String buildTokenUrl(String serverAddress) throws MalformedURLException {
        ...
        // If the address points to a marketplace or plugin endpoint,
        // go back to the GLPI root
        int marketplaceIndex = path.indexOf("/marketplace");
        int pluginsIndex = path.indexOf("/plugins");

        int rootEndIndex = -1;
        if (marketplaceIndex >= 0 && pluginsIndex >= 0) {
            rootEndIndex = Math.min(marketplaceIndex, pluginsIndex);
        } else if (marketplaceIndex >= 0) {
            rootEndIndex = marketplaceIndex;
        } else if (pluginsIndex >= 0) {
            rootEndIndex = pluginsIndex;
        }

        if (rootEndIndex >= 0) {
            path = path.substring(0, rootEndIndex);
        }
