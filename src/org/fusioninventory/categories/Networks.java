...
public class Networks
    ...
    public Networks(Context xCtx) {
        ...
        FusionInventory.log(this, "dns2=" + StringUtils.int_to_ip(dhcp.dns2), Log.VERBOSE);
        FusionInventory.log(this, "gateway=" + StringUtils.int_to_ip(dhcp.gateway), Log.VERBOSE);
        c.put("IPGATEWAY",StringUtils.int_to_ip(dhcp.gateway));
        FusionInventory.log(this, "ipAddress=" + StringUtils.int_to_ip(dhcp.ipAddress), Log.VERBOSE);
        c.put("IPADDRESS",StringUtils.int_to_ip(dhcp.ipAddress));
        FusionInventory.log(this, "leaseDuration=" + dhcp.leaseDuration, Log.VERBOSE);
        FusionInventory.log(this, "netmask=" + StringUtils.int_to_ip(dhcp.netmask) , Log.VERBOSE);
        c.put("IPMASK" ,StringUtils.int_to_ip(dhcp.netmask)  );
        FusionInventory.log(this, "serverAdress=" + StringUtils.int_to_ip(dhcp.serverAddress), Log.VERBOSE);
      ...
      /*  
        Enumeration<NetworkInterface> pNetIfs = null;
        NetworkInterface pNetIf = null;
        try {
             pNetIfs = NetworkInterface.getNetworkInterfaces();
        ...
        if(pNetIfs != null) {
            while (pNetIfs.hasMoreElements()) {
                //Category c = new Category(xCtx, "NETWORKS");
                pNetIf = pNetIfs.nextElement();
                if(pNetIf != null) {
                    //c.put("DESCRIPTION", pNetIf.getDisplayName());
                    
                    
                    FusionInventory.log(this, "DESCRIPTION = "+pNetIf.getDisplayName() , Log.VERBOSE);
                   
                    InetAddress inet;
                    Enumeration<InetAddress> inets = pNetIf.getInetAddresses();
                    while ( inets.hasMoreElements()) {
                        inet = inets.nextElement();
                        FusionInventory.log(this, "|-> INET = "+inet.getHostAddress() , Log.VERBOSE);
                        FusionInventory.log(this, "|-> HOST = "+inet.getHostName() , Log.VERBOSE);
