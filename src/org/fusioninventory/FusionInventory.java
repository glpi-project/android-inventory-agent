...
public class FusionInventory extends Activity {
	...
	private boolean menu_getinv = false;

	static void log(Object obj, String msg, int level) {
	...
	class IncomingHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case Agent.MSG_AGENT_STATUS:
				...
				menu_getinv = (msg.arg1 == 0 ? true : false);
				break;
			case Agent.MSG_INVENTORY_FINISHED:

				try {
					mAgentService.send(Message.obtain(null,
							Agent.MSG_INVENTORY_RESULT));
	...
	private ServiceConnection mConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			...
			try {
				...
				mAgentService
				.send(Message.obtain(null, Agent.MSG_AGENT_STATUS));
			} catch (RemoteException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	void doBindService() {
		...
		ComponentName result = startService(new Intent(
		"org.fusioninventory.Agent"));
		if (result != null) {
		...
		if (mIsBound) {
			FusionInventory.log(this, "Connected sucessfully to Agent service",
					Log.INFO);
		} else {
			FusionInventory.log(this, "Failed to connect to Agent service",
					Log.ERROR);
		}
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		// TODO Auto-generated method stub
		MenuItem m = menu.findItem(R.id.menu_getinv);
		m.setEnabled(menu_getinv);
		return true;
	}
